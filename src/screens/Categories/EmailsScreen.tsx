...
export default function EmailsScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      const emailsRes = await axios.get(`${api_url}/api/getemails`, { params: { user_id: currentUserId } }).catch(err => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const updatePayload: any = {
        ...
        user_id: currentUserId,
        data_type: 'email',
        email_title: editFormData.title !== undefined ? editFormData.title : selectedItem.title,
        email: editFormData.key !== undefined ? editFormData.key : selectedItem.key
      };
  ...
  const togglePin = async (itemId: number) => {
    ...
    try {
      await AsyncStorage.setItem('pinned_emails', JSON.stringify(newPinned));
    } catch (e) {
  ...
  const loadPinnedItems = async () => {
    try {
      const pinned = await AsyncStorage.getItem('pinned_emails');
      if (pinned) {
    ...
    return (
      <ScrollView style={styles.detailScrollView}>
        <View style={styles.detailHeader}>
          <Ionicons name="mail" size={40} color="#3B3B98" />
        ...
        <View style={styles.detailField}>
          ...
          <TouchableOpacity
            ...
            onPress={() => copyToClipboard(selectedItem.key, 'Email')}
          >
      ...
      >
        <View style={styles.itemHeader}>
          <View style={styles.itemIconContainer}>
            <Ionicons name="mail" size={24} color="#3B3B98" />
          </View>
          <View style={styles.itemInfo}>
            <Text style={styles.itemTitle} numberOfLines={1}>{item.title}</Text>
            <Text style={styles.itemSubtitle} numberOfLines={1}>{item.key}</Text>
          </View>
          <TouchableOpacity
    ...
    <LinearGradient colors={['#2C2C54', '#474787']} style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <Text style={styles.headerTitle}>E-mails</Text>
        </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>Nenhum email cadastrado</Text>
          </View>
        }
      />
          ...
          >
            <View style={styles.detailModalHeader}>
              ...
              <Text style={styles.detailModalTitle}>
                {editMode ? 'Editar Email' : 'Detalhes do Email'}
              </Text>
            </View>
            {renderDetailContent()}
          </Animated.View>
