...
export default function NotesScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      const notesRes = await axios.get(`${api_url}/api/getnotes`, { params: { user_id: currentUserId } }).catch(err => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const updatePayload: any = {
        ...
        user_id: currentUserId,
        data_type: 'note',
        note_key: editFormData.title !== undefined ? editFormData.title : selectedItem.title,
        note_value: editFormData.description !== undefined ? editFormData.description : selectedItem.description
      };
  ...
  const togglePin = async (itemId: number) => {
    ...
    try {
      await AsyncStorage.setItem('pinned_notes', JSON.stringify(newPinned));
    } catch (e) {
  ...
  const loadPinnedItems = async () => {
    try {
      const pinned = await AsyncStorage.getItem('pinned_notes');
      if (pinned) {
    ...
    if (editMode) {
      return (
        <ScrollView style={styles.detailScrollView}>
          ...
          <TextInput
            value={editFormData.description !== undefined ? editFormData.description : selectedItem.description}
    ...
    return (
      <ScrollView style={styles.detailScrollView}>
        <View style={styles.detailHeader}>
          <Ionicons name="document-text" size={40} color="#3B3B98" />
        ...
        <View style={styles.detailField}>
          ...
          <TouchableOpacity
            ...
            onPress={() => copyToClipboard(selectedItem.description, 'Descrição')}
          >
        ...
        <View style={styles.detailActions}>
          <TouchableOpacity
            ...
            onPress={() => {
              setEditFormData({ title: selectedItem.title, description: selectedItem.description });
              setEditMode(true);
            }}
          >
      ...
      >
        <View style={styles.itemHeader}>
          <View style={styles.itemIconContainer}>
            <Ionicons name="document-text" size={24} color="#3B3B98" />
          </View>
          <View style={styles.itemInfo}>
            <Text style={styles.itemTitle} numberOfLines={1}>{item.title}</Text>
            <Text style={styles.itemSubtitle} numberOfLines={2}>{item.description || 'Sem descrição'}</Text>
          </View>
          <TouchableOpacity
