...
export default function PasswordsScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      const passesRes = await axios.get(`${api_url}/api/getpasswords`, { params: { user_id: currentUserId } }).catch(err => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const updatePayload: any = {
        ...
        user_id: currentUserId,
        data_type: 'password',
        pass_title: editFormData.title !== undefined ? editFormData.title : selectedItem.title,
        password_key: editFormData.key !== undefined ? editFormData.key : selectedItem.key
      };
  ...
  const togglePin = async (itemId: number) => {
    ...
    try {
      await AsyncStorage.setItem('pinned_passwords', JSON.stringify(newPinned));
    } catch (e) {
  ...
  const loadPinnedItems = async () => {
    try {
      const pinned = await AsyncStorage.getItem('pinned_passwords');
      if (pinned) {
  ...
  const sortedPasses = useMemo(() => {
    ...
    const sortItems = (items: any[]) => {
      switch (sortOrder) {
        case 'recent':
          return items; // Já vem ordenado por data do servidor
        case 'oldest':
    ...
    if (editMode) {
      return (
        <ScrollView style={styles.detailScrollView}>
          ...
          <TextInput
            value={editFormData.key !== undefined ? editFormData.key : selectedItem.key}
    ...
    return (
      <ScrollView style={styles.detailScrollView}>
        <View style={styles.detailHeader}>
          <Ionicons name="lock-closed" size={40} color="#3B3B98" />
        ...
        <View style={styles.detailField}>
          ...
          <TouchableOpacity
            ...
            onPress={() => copyToClipboard(selectedItem.key, 'Senha')}
          >
        ...
        <View style={styles.detailActions}>
          <TouchableOpacity
            ...
            onPress={() => {
              setEditFormData({ title: selectedItem.title, key: selectedItem.key });
              setEditMode(true);
            }}
          >
      ...
      >
        <View style={styles.itemHeader}>
          <View style={styles.itemIconContainer}>
            <Ionicons name="lock-closed" size={24} color="#3B3B98" />
          </View>
          <View style={styles.itemInfo}>
            <Text style={styles.itemTitle} numberOfLines={1}>{item.title}</Text>
            <Text style={styles.itemSubtitle} numberOfLines={1}>••••••••</Text>
          </View>
          <TouchableOpacity
    ...
    <LinearGradient colors={['#2C2C54', '#474787']} style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <Text style={styles.headerTitle}>Senhas</Text>
        </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>Nenhuma senha cadastrada</Text>
          </View>
        }
      />
          ...
          >
            <View style={styles.detailModalHeader}>
              ...
              <Text style={styles.detailModalTitle}>
                {editMode ? 'Editar Senha' : 'Detalhes da Senha'}
              </Text>
            </View>
            {renderDetailContent()}
          </Animated.View>
